...
class high_team_Public {
	...
	public function __construct( $high_team, $version ) {
		...
		// add archive & single for high team		
		add_filter( 'template_include', array($this, 'add_high_team_template'));
		
		add_filter('theme_page_templates', array($this,'high_team_page_template'));
																			  
add_filter( 'template_include', array($this, 'wpse_288589_load_plugin_template' ));
		...
		if ( get_post_type() == 'high_team' ) {  
			...
			if ( is_single() ) {
				...
				if ( $theme_file = locate_template( array ( 'archive-high_team.php' ) ) ) {
					$template_path = $theme_file;
				} else { $template_path = plugin_dir_path( __FILE__ ) . 'templates/archive-high_team.php';
				}
			}
		}
	...
	function high_team_page_template( $post_templates) {
	...
	function wpse_288589_load_plugin_template( $template ) {
		...
		if(  get_page_template_slug() === 'high-team.php' ) {

			if ( $theme_file = locate_template( array( 'high-team.php' ) ) ) {
				$template = $theme_file;
			} else {
